...
> 
> ⛔ **`--no-verify` is STRICTLY FORBIDDEN.** Never use it under any circumstance. Pre-commit hooks (lint, type-check, tests) exist to protect the codebase. Bypassing them is a project rule violation.
...
**DON'T**: Use `--no-verify` — **THIS IS STRICTLY FORBIDDEN**. Pre-commit hooks (lint, type-check, tests) MUST always run. No exceptions.
...
- [ ] ⛔ `--no-verify` was NOT used — hooks ran and passed
...
⛔ **Skipping hooks** - Using `--no-verify` is **STRICTLY FORBIDDEN**. If hooks fail, fix the underlying issue — never bypass the hooks.  
